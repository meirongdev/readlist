...
> 日期: 2026-08-04
> 状态: 📐 设计提案，回应 [review-2026-08-04.md](../archive/review-2026-08-04.md)
...
单一 A/B/C/D 字母**保留但降级为 UI 徽章**，且只看**实际参与排序的维度**
（各 preset 加权维的并集，见 [scoring-standard §5.2](scoring-standard.md)）：
`A = 该集合全部 measured`、`B = 有 shrunk 但仍有外部信号`、`C = 只剩本地元数据`、
`D = 该集合里有 unknown`。它不再决定任何准入。

集合取全局并集而不是逐榜算：同一本书在目录页、详情页和各份榜上必须是同一个字母，
逐榜评级会让读者在三个页面看到三个互相矛盾的评价。
...
2. **上 internal 榜 ≠ 公开。** `library-hygiene` 是给库主人看的运维视图，它的行不进公开集合。
...
同一条边界后来也解释了权重滑块为什么撤掉：它只能在**本榜已选出的书内重排**，
要把榜外的书换进来必须重跑选材，而那正是公开面不愿意提供的全库计算入口。
...
它决定 `to-read-next` 有没有内容，且只有库主人能做。
