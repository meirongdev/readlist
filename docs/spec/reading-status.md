...
| `read-and-loved`「我读过且推荐」 | **已读 ∩ 个人评分高**。真人读过的推荐 > 算法分，是公开书单最有说服力的部分 | ⚠️ **尚未上线**（见下） |

### `read-and-loved` 的问题

**它现在卡在两处，两处都得解开才能上线。**

一、数据太薄：实测**已读 ∩ 有库内评分只有 3 本**。两条路：
...
二、这份榜还声明了 `needs: {D: measured}`（深度），而 `D` 维**没有任何生产数据源** ——
它只来自 `labels` 表，全仓唯一往那张表写数据的是 `corpus.Seed`（演示语料），
LLM 标注是 [roadmap](../roadmap.md) 第 6 步。所以即使星级补齐，这份榜在真实库上
仍然是空的。它因此暂未列入 `presets.yaml`（见 requirements FR-31b）。
